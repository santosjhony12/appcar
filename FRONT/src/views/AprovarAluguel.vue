<template>
  ...
  <div class="filters">
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(aluguel) in alugueisFiltrados">
                ...
                <td>
                  <button @click="showModalAutorizar(aluguel.id)">Autorizar</button>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    <div v-if="showModal" class="modal-overlay">
      <div class="modal">
        <h2 class="titulo-modal">Deseja confirmar o aluguel do veículo?</h2>
  
        <div class="buttons">
          <Button :text="'Sim'" :is-loading="isLoading" @click="autorizarAluguel" />
...
const filtroPlaca = ref<string>('');
  ...
  const alugueisNaoAprovados = ref<Aluguel[]>([]);

  const isLoading = ref<boolean>(false);

  const showModal = ref<boolean>(false);
  onMounted(() => {
    getAlugueisNaoAutorizados()
  })
  ...
  const autorizarAluguel = async () => {
      const response = await AluguelService.autorizarAluguel(idAluguelAutorizar.value);
...
watch([filtroNome, filtroPlaca], () => {
  let filtrados = alugueisNaoAprovados.value;
